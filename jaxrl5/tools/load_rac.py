"""Loader for RAC (Reachable Actor-Critic) checkpoints."""
...
def _find_config_path(ckpt_file: Path) -> Path:
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            ...
            "Provide a run_dir with config.json to rebuild RACLearner."
        )
    return config_path
...
def load_rac(
    ...
    *,
...
) -> Tuple[RACLearner, PolicyFn, Dict[str, Any]]:
    """Load a RACLearner checkpoint and return (agent, policy_fn, meta)."""
    if observation_space is None or action_space is None:
        raise ValueError("load_rac requires observation_space and action_space to build a template agent.")
    ...
    if hasattr(serialization, "msgpack_restore"):
        ...
        if isinstance(restored, RACLearner):
            agent = restored
    ...
    if not isinstance(agent, RACLearner):
        raise TypeError(f"load_rac produced {type(agent)} instead of RACLearner")
