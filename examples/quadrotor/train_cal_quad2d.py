...
# NOTE: visualize_policy_trajectory/load_agent currently does not support "cal".
# Minimal follow-up change (outside this prompt scope): add jaxrl5/tools/load_cal.py
# and register it in jaxrl5/tools/load_agent.py + visualize_policy_trajectory.py choices.
#
...
flags.DEFINE_string("project_name", "jaxrl5_quad2d_cal", "wandb project name.")
...
flags.DEFINE_string(
    "results_root",
    "results/QuadrotorTracking2D-v0/jaxrl5_quad2d_cal",
    "Root directory for experiment outputs.",
)
config_flags.DEFINE_config_file(
    "config",
    "examples/quadrotor/configs/cal_quad2d_config.py",
    "Path to CAL hyperparameter configuration.",
    lock_config=False,
)
...
def _make_eval_policy(agent: CALAgent) -> Callable[[np.ndarray], np.ndarray]:
    eval_agent = agent
...
def _save_checkpoint(agent: CALAgent, ckpt_dir: str, step: int) -> str:
...
def _load_checkpoint(path: str) -> CALAgent:
    with open(path, "rb") as f:
        data = f.read()
    restored = None
    if hasattr(serialization, "msgpack_restore"):
        restored = serialization.msgpack_restore(data)
        if isinstance(restored, CALAgent):
            return restored
        if isinstance(restored, dict):
            for value in restored.values():
                if isinstance(value, CALAgent):
                    return value
    return serialization.from_bytes(CALAgent, data)
...
def _run_evaluation(agent: CALAgent, eval_env, step: int, experiment_name: str):
...
def _training_loop(run_dir: str) -> None:
    ...
    agent: CALAgent = globals()[model_cls].create(
        FLAGS.seed,
