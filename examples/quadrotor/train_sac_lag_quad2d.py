...
# NOTE: visualize_policy_trajectory/load_agent currently does not support "sac_lag".
# Minimal follow-up change (outside this prompt scope): add jaxrl5/tools/load_sac_lag.py
...
flags.DEFINE_string("project_name", "jaxrl5_quad2d_sac_lag", "wandb project name.")
...
flags.DEFINE_string(
    "results_root",
    "results/QuadrotorTracking2D-v0/jaxrl5_quad2d_sac_lag",
    "Root directory for experiment outputs.",
)
config_flags.DEFINE_config_file(
    "config",
    "examples/quadrotor/configs/sac_lag_quad2d_config.py",
    "Path to SAC-Lag hyperparameter configuration.",
    lock_config=False,
)
...
def _make_eval_policy(agent: SACLagLearner) -> Callable[[np.ndarray], np.ndarray]:
    eval_agent = agent
...
def _save_checkpoint(agent: SACLagLearner, ckpt_dir: str, step: int) -> str:
...
def _load_checkpoint(path: str) -> SACLagLearner:
    return SACLagLearner.load(path)
...
def _run_evaluation(agent: SACLagLearner, eval_env, step: int, experiment_name: str):
...
def _training_loop(run_dir: str) -> None:
    ...
    agent: SACLagLearner = globals()[model_cls].create(
        FLAGS.seed,
